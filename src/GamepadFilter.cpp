...
void GamepadFilter::handleEvent(PadState& pad, const js_event& ev) {
    ...
    if (ev.type == TYPE_BUTTON) {
        ...
        int key = 0;
        switch (idx) {
            case kBtnA: key = Qt::Key_Return; break;
            case kBtnB: key = Qt::Key_Escape; break;
            case kBtnX: key = Qt::Key_F5; break;
            case kBtnY: key = Qt::Key_Tab; break;
            case kBtnStart: key = Qt::Key_F5; break;
            default: return;
        }
        pad.heldKey = key;
        pad.repeatTimer.restart();
